...
We predict $\hat{i}$ using the Bayes' rule.  Letting $p = \Pr[\hat{i} \neq i]$, then by Fano's inequality,
\[
\log_2 K \leq \frac{H(p) + \frac{1}{2}\log \frac{|I + \Omega|}{|\Omega|}}{1-p}.
\]
where
\[
H(p) = p\log_2 p+ (1-p) \log_2(1-p) \leq 1.
\]

In particular, for $p=1/2$,
\[
\log_2 K \leq 2 + \log \frac{|I + \Omega|}{|\Omega|}.
\]
