...
\text{AvRisk}_{k, r}(\mathcal{F}) = \E[L] = \E[C(\mathcal{F}(\{\hat{F}_y\}_{y \in \mathcal{S}})(X^*), Y^*)].
\end{equation}
...
\centering
\begin{tabular}{cc}
\multirow{3}{*}{\includegraphics[scale = 0.5, clip = true, trim = 0 0 0 0.5in]{extrapolation_figures/illus_rho_0_7.pdf}} & \\
& \includegraphics[scale = 0.5, clip = true, trim = 0 0.8in 0 0.8in]{extrapolation_figures/illus_example1a.pdf}\\
...
\begin{equation}\label{eq:def_condrisk}
...
\begin{equation}\label{eq:condrisk_identity}
...
By the property of conditional expectations, and by
definitions \eqref{eq:def_condrisk} and \eqref{eq:avrisk_EL} and identity \eqref{eq:condrisk_identity} , 
for $Y \sim \pi$, $\hat{F}_Y \sim \Pi_{Y, r}$ and $X \sim F_Y$ we have
\begin{align*}
\text{AvRisk}_{k,r} &= \E[\text{CondRisk}_k((Y, \hat{F}_Y), X)]
\\&= \int \text{CondRisk}_k((y, \hat{F}_y), x) d\pi(y) dF_y(x) d\Pi_{y, r}(\hat{F}_y)
\\&= (k-1) \int \int D((y^*,\hat{F}_{y^*}), x^*, u) u^{k-2} du d\pi(y) dF_y(x) d\Pi_{y, r}(\hat{F}_y)
\\&= (k-1) \int \bar{D}(u) u^{k-2} du,
\end{align*}
where
\[
\bar{D}(u) = \int D((y^*,\hat{F}_{y^*}), x^*, u) \pi(y^*)dy dF_{y^*}(x^*) d\Pi_{y^*, r}(\hat{F}_{y^*}).
