...
Also define
\[
C_{i,j, \ell} = C(y^{(\ell)}, y^{(i)})I\{U_{i, j, \ell} > U_{i, j, i}\}
\]
By definition, we have
\[
\bar{K}(u) = \E[C(Y, Y^*)I\{U_{X^*}(Y) > U_{X^*}(Y^*)\}] = \E[C_{i,\ell} | U_{i, j, \ell} = u].
\]
This means that $\bar{K}(u)$ can be estimated via a $d$-th order polynomial regression
\[
\hat{\beta} = \argmin_{\beta} \sum_{i=1,j=1, \ell=1}^{r_{test}, k_1, k_1} \left(C_i^\ell - \sum_{h=0}^d \beta_h U_{i,j, \ell}^h\right)^2
\]
for the dataset $\{(U_{i,j, \ell}, C_{i,\ell})\}_{i=1,
j=1, \ell=1}^{r_{test}, k_1, k_1}$.  However, this is not possible in
practice because $U_{i,j,\ell}$ are not directly observed.

Instead, we can obtain an unbiased estimate of $\hat{U}_{i, j, \ell}$ via
\[
\hat{U}_{i,j, \ell} = \frac{1}{(k-1)}\sum_{m \neq \ell} I\{M_{i, j}^\ell > M_{i, j}^m\}
\]
...
\[
\hat{U}_{i,j,\ell}^{(2)} = \frac{1}{ k(k-1)} \sum_{m_1\neq m_2 \neq j} I\{M_{i, j}^\ell > M_{i, j}^{m_2}\} I\{M_{i, j}^\ell > M_{i, j}^{m_2}\}.
\]
In general, the U-statistic is
\[
\hat{U}_{i, j, \ell}^{(h)} = \frac{(k-h)!}{k!} \sum_{m_1 \neq \cdots \neq m_h \neq j} \prod_{z=1}^h I\{M_{i, j}^\ell > M_{i, j}^{m_z}\}.
\]

Then, define
\[
\hat{\beta} = \argmin_\beta \sum_{i=1,j=1, \ell=1}^{r_{test}, k_1, k_1} \left(C_i^\ell - \sum_{h=0}^d \beta_h \hat{U}_{i,j, \ell}^{(h)}\right)^2.
\]
...
\fbox{
  \parbox{\textwidth}{
    ...
    \emph{Inputs:}
    \begin{itemize}
    \item Marginal model $\mathcal{M}$.
    \item Label set $\mathcal{S}_1 =\{y^{(1)},\hdots, y^{(k_1)}\}$.
    \item Prior class probabilities $\{\pi_1(y^{(1)}),\hdots, \pi_1(y^{(k_1)})\}$
    \item Training sample $\hat{F}_{y^{(i)}}$ for $i = 1,\hdots, k_1$.
    \item Test sample $(x_1^{(i)},\hdots, x_{r_{test}}^{(i)})$ for $i = 1,\hdots, k_1$.
    \end{itemize}
    ...
    \emph{Algorithm:}
    \begin{enumerate}
    \item For $i = 1,\hdots, k_1$, $j= 1,\hdots, r_{test}$, and $\ell = 1,\hdots, k_1$, compute the margins
    \[
    M_{i, j}^\ell = \mathcal{M}(\hat{F}_{y^{(\ell)}}; \pi_1(y^{(\ell)}))(x_j^{(i)}).
    \]
    \item  For $i = 1,\hdots, k_1$, $j= 1,\hdots, r_{test}$, $\ell = 1,\hdots, k_1$, and $h = 0,\hdots, d$, compute
    \[
    \hat{U}_{i, j, \ell}^{(h)} = \frac{(k-h)!}{ k!} \sum_{m_1 \neq \cdots \neq m_h \neq j} \prod_{z=1}^h I\{M_{i, j}^\ell > M_{i, j}^{m_z}\}.
    \]
    \item For $i = 1,\hdots, k_1$, $j= 1,\hdots, r_{test}$, $\ell = 1,\hdots, k_1$, compute
    \[
    C_{i,j, \ell} = C(y^{(\ell)}, y^{(i)})I\{U_{i, j, \ell} > U_{i, j, i}\}.
    \]
