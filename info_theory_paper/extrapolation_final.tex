...
\textbf{Proof.}
It is clear from the definition \eqref{eq:U_function} that $U$ is
bounded between 0 and 1.  Therefore, let us take $u \in [0,1]$.  We have
\begin{align*}
\Pr[U \leq u] &= \Pr[U_{Y, \hat{F}_Y}(x) \leq u]
\\&= \Pr\left[\int 
I\{\mathcal{M}(\hat{F}_{Y})(x) > \mathcal{M}(\hat{F}_{w})(x)\}
d\Pi_{w, r}(\hat{F}_w)
d\pi(w)
\leq u\right]
\\&= \int_{\mathcal{Y}} 
