...
\caption{
(a) The joint distribution of $(X, Y)$ is bivariate normal with correlation $\rho$.
...
\caption{The distribution of the classification risk for $k = 2,3,\hdots, 10$.  
...
\caption{
(a) The level curves of the function $U_y(x)$ in the bivariate normal model.
(b) The function $\bar{K}(u)$, which gives the cumulative distribution function of the random variable $U_Y(X)$.}\label{fig:toy3}
\end{figure}
...
\caption{
The (a) average risk, (b) $\bar{K}(u)$ function for $k = 2,\hdots, 7$ for the bivariate normal model with $\rho \in \{0.3, 0.5, 0.7, 0.9\}$.
(c) The $d$-degree $\ell_\infty$ polynomial approximation error for $\bar{K}(u)$ for the bivariate normal model with $\rho \in \{0.3, 0.5, 0.7, 0.9\}$.
}\label{fig:toy4}
\end{figure}
