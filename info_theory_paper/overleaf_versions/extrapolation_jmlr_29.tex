...
\[Acc_2 (x_\ell^{(i)}; m) := P( m_{y^{(i)}}(x_\ell^{(i)}) > m_Y (x_\ell^{(i)})).\]
...
\[\mathrm{Acc}_k (x_\ell^{(i)} ; m_{y^{(i)}}) := P( m_\ell^{(i,i)} > \mbox{max}_{j \neq i}\ m_\ell^{(i,j)} \}) = \mathrm{Acc}_2(x_\ell^{(i)})^{k-1}, \]
recalling that we defined $m_\ell^{(i,j)} = m_{y^{(j)}}(x_\ell^{(i)})$.
Given estimated accuracy values $a_\ell^{(i)} = \widehat{Acc}_2 (x_\ell^{(i)})$, $\ell =1,\hdots, r$, $i = 1,...,k$, 
we can average the $K-1$ powers: 
\[\widehat{\mathrm{AGA}}_K  = \widehat{\E[\mathrm{Acc}_K (x_\ell^{(i)})} ]= \frac{1}{rk} \sum_{i = 1}^k \sum_{\ell=1}^r (1-(1-a_\ell^{(i)})^{K-1}) \]
Note that if we have noisy but unbiased estimates of $\mathrm{Acc}_2(x_\ell^{(i)} ; m_{y^{(i)}})$,
then the estimates for $\mathrm{Acc}_K(x_\ell)$ and $\widehat{\mathrm{AGA}}_K$ will be upward biased
...
\[\mathrm{Acc}_2(x_\ell^{(i)}) = \int_{-\infty}^{m_\ell^{(i,i)}} \hat{f}_\ell^{(i)}(m)dm.\]
\end{enumerate}
