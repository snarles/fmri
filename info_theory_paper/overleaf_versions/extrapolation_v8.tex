...
$|\mathcal{S}_1| < \cdots < |\mathcal{S}_K|, \mathcal{S}_i \subset \mathcal{Y}$,
where in the $i$-th task, one constructs the classification rule
$h^{(i)}:\mathcal{X} \to \mathcal{S}_i$.  Supposing that $(X, Y)$ have
a joint distribution, define the generalization accuracy for the
$i$-th problem as
\[
\text{GA}^{(i)} = \Pr[h^{(i)}(X) = Y|Y \in \mathcal{S}_i].
\]
...
which is the \emph{label set} $\mathcal{Y}$.  [[Should be $\mathcal{S}$? YB]] 
However, under this setup, it is usually assumed that the
label set $\mathcal{Y}$ [[Should be $\mathcal{S}$? YB]] is a finite and specific set of categories, which is known in advance. [[Citations YB]] For human recognition systems, a fixed and finite label set is not clearly not the case: humans can easily learn to recognize new people, and the facial recognition system is successful because we can tell apart accurately almost any set of, say, 100 individuals.  Similarly, we think about success of recognition systems as a property that is more general than accuracy on any individual set of classes. Therefore we need to extend the notion of accuracy to deal with multiple but related classification tasks.
[[Some citations of people doing mildly similar things YB]] 
...
average risk $\text{AvRisk}_k$ as a function of $k$. 

\section{Extrapolation}

The section is organized as follows.  We begin by
