...
% Whoops, this below part was not necessary
%In
%engineering contexts, where the accuracy of the classifier matters a
%great deal, and where even a 1 percent absolute different in accuracy
%can be practically significant, there is still a need to find the best
%classifier for the given problem.  However, for scientific
%applications, such as the one described, where the purpose of the
%classifier is to infer other quantities of interest (areas specialized
%for a task,) it is often seen that the final conclusion is not too
