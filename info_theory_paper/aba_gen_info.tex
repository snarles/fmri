...
are pair-sampling, with $k$ pairs for the task sampling scheme.  The
functions $\vec{u}$ and $\vec{v}$ are stochastic: let $J$ be an
independent uniform variate drawn from $\{1,\hdots, k\}$.  We have
\[
\vec{u}((\bx_1,\by_1),\hdots, (\bx_k, \by_k)) = (\bx_1,\hdots, \bx_k, \by_J).
\]
\[
\vec{v}((\bx_1,\by_1),\hdots, (\bx_k, \by_k)) = J.
\]
...
The functions $\vec{u}$ and $\vec{v}$ are stochastic: let $J$ be an
independent uniform variate drawn from $\{1,\hdots, M\}$.  We have
\[
\vec{u}((x_1,y_1),\hdots, (x_{Mk}, y_{Mk})) = (x_1,\hdots, x_{Mk}, y_{(J-1)k + 1},\hdots, y_{(J-1)k}).
\]
\[
\vec{v}((x_1,y_1),\hdots, (x_k, y_k)) = J.
\]
The cost function $C$ is the zero-one loss.
\end{itemize}
...
$\bX_J$.  The reciever observes the message
\[
\bY = (Y_{(i-1)k+1}, \hdots, Y_{(i-1)k}).
\]
Then, the decoding task is for the reciever to guess the index of the
message, $J$, based on $\bY$.  It is a consequence of a famous result
...
marginal distributions of $\bX$ and $\bY$.  This is clearly met in the
regression and classification case.  In the identification case, it is
also met, since if $\bX \perp \bY$ then $\by_J$ is independent of $J$.
