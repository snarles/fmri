\title{Inference of mutual information and its generalizations from data}
...
of \emph{mutual information}: if $\bX$ and $\bY$ have joint density
$p(\bx, \by)$, then the mutual information is defined as
\[
I(\bX;\bY) = \int p(\bx, \by) \log \frac{p(\bx, \by)}{p(\bx)p(\by)}dxdy.
\]
where $p(\bx)$ and $p(\by)$ are the marginal densities.  The word
...
$I(\bX;\bY)$, opens the possibility of computing, estimating or inferring
the quantity for $(\bX, \bY)$ pairs of interest in the natural world.
...
firing rate of particular motor neurons, $Y$. Once estimates are
