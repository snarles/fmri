...
information is used to test for conditional independence (DeCampos 2006), to
...
use as an objective function for training neural networks (Linkser 1989), for
feature selection in machine learning, and even as an all-purpose
nonlinear measure of ``correlation for the 21st century'' (Speed 2011.)
...
estimation of entropy and mutual information, see (Beirlant 1997) for a
review.
...
in applications with high-dimensional data (Beirlant 1997).  One such
application includes multivariate pattern analysis (MVPA), an area of
neuroscience research pioneered by Haxby (2001), which studies how
...
most commonly employed notion of generalization error (Naselaris
2011).  In an exemplar-based classification, there is no need to
...
classification error (Friedman et al 2008).  One creates a
...
information of the confusion matrix, as introduced by Treves (1999).
...
As noted in the literature (Quiroga et al. 2009), such discriminative
...
considerations (Gastpar 2009.)  When $\log(k) \ll I(X; Y)$, we say
...
model.  Gastpar et al. (2009), studied the nonparametric estimator
\[
...
manifold\footnote{In situations where $(X, Y)$ lie on a manifold, one
  ...
  estimation (Theunnissen and Miller, 1991). }.  In section 2.1 we
...
appears as the error rate of an orthogonal constellation (Tse 2005).  What
...
environmental stimuli $X$ (Banerjee et al 2011).  We choose the naive Bayes for the
...
extrapolating from learning curves (Cortes et al. 1999).  Furthermore,
...
[2] Borst, A. and Theunissen, F. E. ``Information theory and neural coding''
