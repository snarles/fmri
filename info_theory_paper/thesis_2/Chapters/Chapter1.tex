...
Kay et al. test two different models for how visual information is
\emph{represented} in the V1 area of the brain.  Models of this type
are called \emph{encoding models} in the neuroscience literature.  One
encoding model is the \emph{retinotopic} model, in which the raw image
$\vec{z}_i$ is represented by a library of \emph{center-surround}
receptive fields, which are image filters composed of the difference
of two circular distributions centered around the same point.  The
second encoding model that Kay et al. evaluated was the \emph{Gabor
  filter} model, consisting of sinusoidal filters which are sensitive
to position, frequency, and orientation (Figure \ref{fig:gabor}).
