...
constrained to a polyhedral cone.  As the distribution of the training
data moves within $(k_1-1)$-dimensional space, the projection of the
mean of the data distribution onto the cone traces out a discontinuous
curve as the signal-to-noise parameter $\sigma$ for generating the
data is varied.  This observation suggests that carefully adjusting
the placement of the basis knots could improve the worst-case
goodness-of-fit to the multivariate gaussian model (or any parametric
data model) by flattening out the ``peaks'' seen in Figure
\ref{fig:sim_study}, but we leave this idea to future work.

Compared to the regression method, the error curves of the KDE methods
are more easily interpretable.  In the first six plots of
\ref{fig:sim_study}, all of the error curves contain a prominent mode
located at the higher accuracy (higher SNR) settings of the
simulation.  This suggests that for these settings, the bias of both
KDE-UCV and KDE-BCV is worst 
