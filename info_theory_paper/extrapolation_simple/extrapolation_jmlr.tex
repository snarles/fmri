...
\centering
\begin{tabular}{c}
\begin{myfont}Predicting $\text{AGA}_{1672}$ from $k_1= 400$ classes\end{myfont}\\
...
We ran a simulation study in order to understand how well the proposed extrapolation method
scales with different numbers of classes, $k_1$, and on problems of varying difficulty, on a large multi-class dataset,
as well as how the performance depends on the choice of basis.
Similarly to the toy example, we generated data according to a mixture of isotropic multivariate gaussian distributions.
We generated the labels $Y \sim N(0, I_{10})$, and the distribution of examples for each label is
$X|Y \sim N(Y, \sigma^2 I_{10})$, where $\sigma$ is a noise parameter that we vary in the simulations.
Analagously to the real-data example, we consider a 1-nearest neighbor classifier which is given a single
training instance per class.

All of the spline bases that we consider are linear spline bases, of the form $h_\ell(u) = [u - t_\ell]_+$ for some set of knots $\{t_\ell\}_{\ell = 1}^m$.  However, we vary the number of knots, and the spacings of the knots.  One type of spacing is \emph{linear spacing}, which chooses knots $\{t_\ell\}_{\ell = 1}^m$ according to
\[
t_\ell = \frac{\ell}{m+1}.
\]
We also consider \emph{quadratic spacing}, which has an increased density of knots near 1:
\[
...
In all simulations, we consider predicting the average accuracy for $k = 2000$ classes.  
We vary the parameters $k_1$, the number of classes in the source task, and $\sigma$, 
which controls the noise level, and hence the true average accuracy.
The results are displayed in figure \ref{fig:sim_study}.  
...
We see that the performance curves of the spline bases can be roughly clustered into two groups.
The {\tt lin2} spline basis with linear spacing and $m=100$ forms a group on its own,
while the other spline bases form the other group.
The first group enjoys better RMSE for $k_1 < 150$; however, for $k_1 > 400$ the second group has far better extrapolation peformance.
The relatively poor performance of the {\tt lin2} spline basis can be explained by the fact that 
the {\tt lin2} spline basis cannot approximate the true $D(u)$ function (bottom left of figure) to a sufficiently high resolution.
When there is insufficient data to fit $D(u)$ with a more accurate model, such as when $k_1 < 150$,
this may not be a disadvantage, but as more data becomes available and the variance of estimation
decreases, models which can better fit $D(u)$ (and hence have lower bias) 
demonstrate better convergence properties.
However, it is worth noting that the {\tt lin2q} basis, which also has $m=100$ knots, but which
are quadratically spaced rather than linearly spaced, matches the performance of much larger spline bases
up to $k_1 = 2000$.  This suggests that a small spline basis with $m << k_1$ can perform almost as well
as an optimally sized spline basis, as long as appropriate spacing is used for the knots.
 
In the bottom right of figure \ref{fig:sim_study}, we see how the error depends on the true parameter 
$\sigma=\{0.3,\hdots,0.7\}$ while fixing $k_1 = 500$,
which allows us to evaluate the reliability of the extrapolation across a wide range of possible signal-to-noise ratios.
...
\end{tabular}
\caption{ Comparison of various linear spline bases for extrapolation,
...
The extrapolation method of \citep{Kay2008a} can be described in our
