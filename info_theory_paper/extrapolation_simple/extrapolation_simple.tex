...
known.  
However, applying this result in practice to estimate AGAk requires specifying an estimator for the unknown function D. 
We propose using non-parametric regression in Section \ref{sec:extrapolation_estimation}.
...
Let $B_1,\hdots, B_s$ be a set of candidate bases, with $B_i = \{h_u^{(i)}\}_{u=1}^{m_i}$.  Ideally, we would like our model selection procedure to choose the $B_i$ that obtains the best RMSE on the extrapolation from $k_1$ to $k_2$ classes.  We can make the problem if we assume that a basis $\{h_u\}$ is that is a good fit to the discrimination function $D(u)$ should work well for extrapolation regardless of the particular values of $k_1$ or $k_2$.  Therefore, we might be settle for a model selection procedure that picked the best basis $B_i$ for a different extrapolation--say, from $\frac{k_1}{2}$ source classes to $k_1$ target classes.  Even then, we would need to know the joint distribution of the data to evaluate the population RMSE.  However, we can use the ``bootstrap principle'' to approximate the joint distribution of the data with the observed data.  What this amounts to is a resampling-based model selection approach, where we perform extrapolations from $\lfloor \frac{k_1}{2} \rfloor$ classes to $k_1$ classes, and evaluate methods based on how closely the predicted $\widehat{\text{ABA}}_{k_1}$ matches the test accuracy $\text{ATA}_{k_1}$.  To elaborate, our model selection procedure is as follows.

\begin{enumerate}
\item For $\eta=1,\hdots,L$ resampling steps:
\begin{enumerate}
\item Subsample $\mathcal{S}_{\lfloor \frac{k_1}{2} \rfloor}^{(\eta)}$ from $\mathcal{S}_{k_1}$ uniformly with replacement.
\item Compute average test accuracies $\text{ATA}_{\lfloor \frac{k_1}{2} \rfloor}^{(\eta)}$ from the subsample $\mathcal{S}_{\lfloor \frac{k_1}{2} \rfloor}^{(\ell)}$.
\item For each candidate basis $B_i$, with $i = 1,\hdots, s$:
\begin{enumerate}
\item Compute $\hat{\beta}^{(i,\eta)}$ by solving the least-squares problem
\[\hat{\beta}^{(i,\eta)} = \argmin_\beta \sum_{k=2}^{\lfloor \frac{k_1}{2} \rfloor} \left( (1 - \text{ATA}_k^{(l)}) - \sum_{\ell=1}^{m_i} \beta_\ell H_{\ell, k}^{(i)}\right)^2.\]
\item Estimate $\widehat{\text{AGA}}_{k_1}^{(i,\eta)}$ by
\[
\widehat{\text{AGA}}_{k_1}^{(i,\eta)} = \sum_{\ell=1}^{m_i} \beta_\ell H_{\ell, k_1}^{(i)}.
\]
\end{enumerate}
\end{enumerate}
\item Select the basis $B_{i^*}$ by
\[
i^* = \text{argmin}_{i=1}^s \sum_{\eta=1}^L (\widehat{\text{AGA}}_{k_1}^{(i,\eta)} - \text{ATA}_{k_1})^2.
\]
...
to the model trained for the $j$'th class $m_{y_j}$. 
For each feature-vector $x_i$, the density of wrong-class scores is estimated by smoothing the observed scores
with a kernel function $K(\cdot,\cdot)$ with bandwidth,
\[\hat{f}_i(m) = \frac{1}{k_1-1} \sum_{j\neq i} K_h(m_{y_j}(x_i), m).\]
An estimate of favorability for $x_i$ against a
single competitor is obtained by integrating the density below the observed true score $m_{y_i}(x_i)$, 
\[acc_2(x_i) = \int_{0,m_{y_i}(x_i)}^\infty \hat{f}_i(m)dm,\]
and the probability of accurate classification against $K-1$ random
competitors is $acc_{K}(x_i) = (acc_2(x_i))^{K-1}$. The average of these probabilities is the estimate for generalization accuracy
\[\hat{AGA}_{K}^{(KDE)} = \frac{1}{k_1} \sum_{i}^{k_1} acc_{K}(x_i).\]
...
this, consider a feature vector $x_i$ that is correctly classified in the original class set. That is, $m_{y_i}(x_i)>m_{y_j}(x_i)$ for every
$j\neq i$. If we use for $\hat{f}_i$ the unsmoothed empirical density,
$acc_2(x_i)=1$ and therefore $acc_K(x_i) = 1$ as well. The method relies on smoothing of each class to generate a the tail density that exceeds $m_{y_i}(x_i)$, and therefore it is highly dependent on the choice of kernel bandwidth. 
...
%0.10 in all eight combinations of $(k_1,k_2)$.
% bias of our method?
We see in Figure \ref{fig:sim_study} that the cross-validated regression
...
The kernel-density method produces comparable results, but can be seen to depend strongly on the choice of bandwidth selection: KDE-UCV and KDE-BCV show very different performance profiles, although they differ only in the choice between unbiased and biased cross-validation, respectively.
...
CV-reg shows comparable bias for the 500 to 10000 extrapolation,
...
The face-recognition example takes data from the ``Labeled Faces in the Wild'' dataset (\cite{LFWTech}), where we selected the 1672 individuals with at least 2 face photos.  We form a
...
In order to identify a new photo $\vec{z}^*$, we obtain the feature
...
\hline
$k_1$ & CV-reg & KDE-BCV & KDE-UCV \\\hline
...
$k_1$ & $k_2$ & Classifier & True & CV-reg & KDE-BCV & KDE-UCV \\ \hline
...
moments. Code for the methods and the simulations can be found in [CHARLES TODO].
...
Our CV-reg method adopts a non-parametric model of the discriminability function $D(u)$, in the sense that $D(u)$ was defined via a spline expansion.   However, an alternative approach is to assume a parametric family for $D(u)$ defined by a small number of parameters.  In forthcoming work, we show that under certain limiting conditions, $D(u)$ is well-described by a two-parameter family.  This substantially increases the efficiency of estimation in cases where the limiting conditions are well-approximated.
...
\acks{We thank Jonathan Taylor, Trevor Hastie, John Duchi, Steve
  Mussmann, Qingyun Sun, and Robert Tibshirani, and Gal Elidan for useful discussion.  CZ
