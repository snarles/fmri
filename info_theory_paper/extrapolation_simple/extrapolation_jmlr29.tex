...
1,\hdots, k$.
However, later on we will relax these assumptions on the sampling of the training sets,
...
%post-FP
However, in the case of \emph{randomized} classification, as is the case in face recognition (with faces drawn from a larger population) or a large multi-class problem where only an arbitrary subset of labels has been collected, it would be appropriate to model
the labels $\{Y^{(i)}\}_{i=1}^k$ and associated score
functions as being randomly sampled from their respective distributions.
%EDIT023
