...
%\[Y^* \sim \pi,\, M_{Y^*}|Y^* \sim \nu_{Y^*},\, X^*|Y^* \sim F_{Y^*},\, \]
%\[Y' \sim \pi,\, M_{Y'}|Y' \sim \nu_{Y'}\, X'|Y' \sim F_{Y'},\, \]
With this notation, we have abstracted away the fact that the model $M_Y$ is trained using training data.  
For purposes of our framework, it is sufficient that $M_Y$ is obtained in a way so that it is independent of the other labels, and that it is independent of a test instance $X$ drawn from $F_Y$. %EDIT006
Due to this abstraction, our framework can also accommodate scenarios where the classes have differing or stochastically determined numbers of training instances:
it is not necessary, as in our earlier definitions, to have the same $r_{train}$ instances for every class, as long as the numbers of training instances for the different labels are conditionally independent. %EDIT007
