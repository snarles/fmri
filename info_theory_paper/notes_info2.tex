...
\[
\text{ABA}_k = \frac{1}{k} \int \left[\prod_{i=1}^k p(x_i) dx_i \right] \int dy \frac{\sum_{i=1}^k p(y|x_i)^2}{\sum_{i=1}^k p(y|x_i)}.
\]
...
In this case, the mutual information $\text{I}[p(x,y)]$ is equal to the joint entropy $\text{H}[p(x,y)]$.
...
&= \int \left[\prod_{i=1}^k p(x_i, w_i) dx_i dw_i \right] \int dy dz \frac{\sum_{i=1}^k p(y, z|x_i, w_i)^2}{\sum_{i=1}^k p(y, z|x_i, w_i)}
\\&= \int \left[\prod_{i=1}^k p(x_i, w_i) dx_i dw_i \right] \int dy \frac{\sum_{i=1}^k p(y|x_i)^2}{\sum_{i=1}^k p(y|x_i)} \int p(z|y) dz
\\&= \int \left[\prod_{i=1}^k p(x_i) dx_i \right] \left[\prod_{i=1}^k \int dw_i p(w_i|x_i)\right]\int dy \frac{\sum_{i=1}^k p(y|x_i)^2}{\sum_{i=1}^k p(y|x_i)}
\\&= \int \left[\prod_{i=1}^k p(x_i) dx_i \right]\int dy \frac{\sum_{i=1}^k p(y|x_i)^2}{\sum_{i=1}^k p(y|x_i)} = \text{ABA}_k[p(x,y)].
\end{align*}
