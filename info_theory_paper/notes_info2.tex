...
\nabla \mathcal{L}_{Q^*}(t) = 1 + \lambda + \lambda \log Q^*(t) + \nu t^{k-1}.
\end{equation}
...
explicitly by \eqref{eq:Qstareq} for some $\lambda > 0$, $\nu < 0.$
...
by \eqref{eq:Qstareq} for some $\lambda \geq 0$, $\nu < 0$.  It remains
to determine the values of $\lambda$ and $\nu$.

Reparameterize $\alpha = \exp[-1-\lambda^{-1}]$ and $\beta = -\nu\lambda t^{k-1}$.
Therefore,
\[
...
Therefore, we know that for any $\iota > 0$, there is a unique $\beta > 0$
so that $I[Q^*] = \iota.$
