...
Then defining 
\begin{equation}\label{eqref:veq}
...
Let us define
\[
V_{i,j} = \sum_{\ell\neq i} I(M_i(\hat{F}_1,\hdots, \hat{F}_k, y^{(i, j)})  > M_\ell(\hat{F}_1,\hdots, \hat{F}_k, y^{(i, j)})),
\]
...
\[
\text{maximize }\ell(\eta) \text{ subject to }\frac{d\hat{\eta}}{du} > 0.
\]
...
as
\[
\text{maximize }\ell(\eta) \text{ subject to }\int u^{k-1} \eta(u) du = \hat{p}_k^{UN}.
\]
...
succeeds in optimizing the problems where the dimension of the discretized $\eta$ is as large as 100,000 (Hong 2014, Domahidi 2013.)}.
