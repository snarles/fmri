...
the empirical marginals of the $k$ classes, $\hat{F}_1,\hdots, \hat{F}_k$;  for each class, the classifier assigns a \emph{margin} or \emph{score} to the \emph{query point} $y \in \mathcal{Y}$.  A higher score $\mathcal{M}_i(\hat{F}_1,\hdots, \hat{F}_k, y)$ indicates a higher estimated probability that $y$ belongs to the $k$-th class.  
...
Let us specialize to the case of a generative classifier, with margin $\mathcal{Q}$.
...
Let $\mathcal{Q}$ be a single-distribution margin, and let $\mathbb{F}$, $\hat{\mathbb{F}}(F)$ be a distribution on $\mathcal{P}(\mathcal{Y}).$
Further assume that
...
\noindent\textbf{Theorem 3.3.} \emph{ Let $U$ be defined as in Theorem
  ...
  meta-distribution $\mathbb{F}$ and a margin $\mathcal{Q}$ such
...
Let $\mathcal{Q}$ be the margin of a uniformly generative classifier, and 
