...
Let $x^{(i)}$ denote the native $128 \times 128$-pixel representation of the image
(i.e. a $16384$-dimensional vector with entries between 0 and 1.)
In the study, Kay et al. parameterized the raw stimuli $x^{(i)}$ using a library of
Gabor wavelet filters, $g_1,\hdots, g_{10921}$, where each filter is a nonlinear mappping
\[
g_j: \mathcal{X} \to \mathbb{R}.
\]
Also let $\vec{g}$ denote the vector-valued function
\[
\vec{g}(x) = (g_1(x),\hdots, g_{10921}(x)).
\]
...
\[
e_{gen}(f) = \Pr[f(Y, x^{(1)},\hdots, x^{(k)}) \neq i|Y \sim p(y|x^{(i)})],
\]
and once again the Bayes rule
\[
e_{Bayes}(x^{(1)},\hdots, x^{(k)}) = \inf_f \Pr[f(Y, x^{(1)},\hdots, x^{(k)}) \neq i|Y \sim p(y|x^{(i)})].
\]
Also, the Bayes identification rule which attains the minimal error can be given explicitly,
\[
f_{Bayes}(y, x^{(1)},\hdots, x^{(k)}) = \argmin_{i=1}^k p(y|x^{(i)}).
\]
The average Bayes error is defined by \eqref{eq:abe} as before.
...
e_{ABE, k} \approx \pi_k(\sqrt{2 I(X; Y)})
\]
holds under the same conditions on $p(x, y)$ as before.
...
\item Train an identification rule $f_b$ using the training set pairs $(x^{(i)}, y^{(i)})$.
...
Now, choosing a level $\alpha$, let $\bar{e}$ be the $(1-\alpha/2)$th
quantile and $\underline{e}$ be the $(\alpha/2)$th quantile of
