...
Then
\[
Z_i = \text{blah blah blah}
\]
and it follows that $Z_i$ are asymptotically normal, with
\[
\begin{bmatrix}
Z_*\\
Z_1\\
...
\begin{itemize}
\item For each $d = 1,2,\hdots$ has an associated bivariate distribution $b_d(x,
y)$ and the components $(X_i^{[d]}, Y_i^{[d]})$ are drawn iid from
$b(x, y)$ for $i = 1,\hdots, d$.
\item $I(X^{[d]}, Y^{[d]}) = c$ is constant
\item $H(X_i^{[d]})$ and $H(Y_i^{[d]})$ are constant.
\end{itemize}
Then, as $d \to \infty$, the misclassification probability
\[
\text{MC} = \Pr[Z_* > ...]
\]
satisfies
