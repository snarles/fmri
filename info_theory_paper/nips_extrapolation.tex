...
a non-empty set of \emph{training} measurements $\{y^1,\hdots, y^n\}$, and a query measurement $y$.
In settings where the parameter vector $x$ is not observed, then only scoring rules with a trivial dependence on $x$ can be implemented.
A scoring-based classifier defines
 \[
f^{(t)}(y) = \argmax_i \mathcal{Q}(x^{(i)}, \{y^{(i), 1},\hdots, y^{(i), n_i}\}, y).
\]
...
for any $x$ and training subset $S = \{y^1,\hdots, y^n\}$,
\begin{equation}\label{eq:tie}
\Pr[\mathcal{Q}(x, S, Y) = \mathcal{Q}(x, S, Y')] = 0\text{ for }Y, Y' \stackrel{iid}{\sim} p(y|x).
\end{equation}
...
\[
\mathcal{Q}_{QDA}(x, \{y^1,\hdots, y^n\}, y) = -(y - \bar{y})^T S^{-1} (y-\bar{y})
\]
where $\bar{y} = \frac{1}{n}\sum_{i=1}^n y^i$ and $S = \frac{1}{n}\sum_{i=1}^n (y^i - \bar{y})(y^i - \bar{y})^T$.
In Naive Bayes, the scoring rule is
\[
\mathcal{Q}_{NB}(x, \{y^1,\hdots, y^n\}, y) = \sum_{i=1}^n \log \hat{f}_i(y_i)
\]
...
We will show that recognition systems based on certain implementations of $k$-nearest neighbors, LDA, one-vs-one, or one-vs-all classifiers satisfy this definition of separability.  Let $S_{i, t} = \{y^{(i),1 }, \hdots, y^{(i), n_i}\}$ denote the set of measurements of species $i$ at time $t$.

\textbf{Definition 2.2.}(i)  Define a binary classifier $\mathcal{B}$ as a binary-valued mapping with three arguments: two training sets $\{y^1,\hdots, y^n\}$ and $\{w^1,\hdots, w^m\}$, and a query $y$.  A one-vs-one (OVO) recognition system is defined by
\[
f_t(y) = \argmax_{i=1}^{k_t} \sum_{j \neq i} I(\mathcal{B}(S_{i, t}, S_{j, t}, y)=0),
\]
resolving ties arbitrarily.

(ii) Define a binary scoring rule $\mathcal{D}$ as a real-valued mapping with three arguments:
two training sets $\{y^1,\hdots, y^n\}$ and $\{w^1,\hdots, w^m\}$, and a query $y$.
A one-vs-all (OVA) recognition system is defined by
\[
f_t(y) = \argmax_{i=1}^{k_t} \mathcal{D}(S_{i, t}, \cup_{j\neq i} S_{j, t}, y).
\]

(iii) Let $d$ be a distance metric on $\mathcal{Y}$.
Let $D_t(y)$ be the multiset of distances $D_t(y) = (d(y, y^{(i), j}))_{i=1, j=1}^{k_t, n_i}$,
and let $d_{t, m}(y)$ denote the $m$th smallest value in $D_t(y)$.
A kNN recognition system is defined by
\[
f_t(y) = \argmax_{i=1}^{k_t} \sum_{j=1}^{n_i} I(d(y, y^{(i), j}) \leq d_{t, m_t}(y)).
\]
where $m_1,m_2, \hdots$ is a predefined sequence of neighborhood sizes,
resolving ties arbitrarily.

(iv) Assume WLOG that $y_1 = 1$ for all $y \in \mathcal{Y}$, and
let $B^t$ be a $p \times k_t$ matrix which minimizes the log-likelihood
\[
\sum_{i=1}^{k_t} \sum_{j=1}^{n_i} \langle y^{(i), j}, B^t_j \rangle - \log\left[\sum_{\ell=1}^{k_t} \exp[\langle y^{(i), j}, B^t_\ell \rangle]\right].
\]
...
(v) Let $\bar{y}^{(i) , t}= \frac{1}{n}\sum_{j=1}^{n_i} y^{(i), j}$, and let $S_t = \sum_{i=1}^{k_t} \sum_{j=1}^{n_i} (y^{(i), j} - \bar{y}^{(i), t})(y^{(i), j} - \bar{y}^{(i), t})^T$.  An LDA recognition system is defined by
\[
f_t(y) = \argmax_{i=1}^{k_t} -(y-\bar{y}^{(i), t})^T S_t^{-1} (y-\bar{y}^{(i), t}).
\]

\textbf{Theorem 2.1.} \emph{(i) OVO recognition systems are separable; (ii) OVA recognition systems are separable;
(iii) a kNN recognition system with neighborhood size $m_t = O(n^\alpha)$ is separable; (iv) multinomial logistic regression recognition systems are separable; (v) LDA recognition systems are separable.}
