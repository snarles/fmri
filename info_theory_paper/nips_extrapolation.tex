...
the number of training observations $y^{(i), 1},\hdots, y^{(i), r_1}$
from each class.  The filtration at time $t$ is defined as the
$\sigma$-algebra induced by observations from species $x^{(1)},
\hdots, x^{(k)}$, hence $\mathcal{F}_t = \sigma\{(x^{(i)}, y^{(i),
  j})\}_{i=1, j=1}^{t, r}$.
...
class:
\[
q^{(i)} = \mathcal{Q}(x^{(i)}, y^{(i), 1}, \hdots, y^{(i), r}).
\]
The classifier $f^{(t)}$ is defined
\[
...
Define the \emph{conditional accuracy} function $u: \mathcal{X} \times
\mathcal{Y} \to [0,1]$ which maps a data point $(x, y)$ to its
conditional accuracy,
\[
u(x, y) = \Pr[\mathcal{Q}(x, y) > q(X, y)|X \sim p(x)].
\]
