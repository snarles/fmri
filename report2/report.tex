...
Here we consider asymptotics of $MC(\Sigma, k)$, for the special case
$\Sigma = \frac{c}{d}I_d$ for some $c > 0$, as the dimension
$d \to \infty$.

\textbf{Theorem.}
There exists a function $m(c, k)$ for $c > 0$, and $k \in \{1, 2, \hdots\}$, such that
\[
\lim_{d \to \infty} MC(\frac{c}{d}I_d, k) = m(c, k)
\]

\emph{Proof.}
Given fixed $k$, for arbitrary $\epsilon > 0$, we can find $d$ sufficiently large such that with probability greater than $1-\epsilon$,
\[
...
Hence given an appropriate random basis $e_1,\hdots, e_k$, we can write
\[
\mu_i = \sqrt{c} e_i + O(\epsilon)
\]
with high probability.  Here the $O(\epsilon)$ is understood to mean a
quantity bounded in magnitude by a universal constant (depending only on $c$) times
$\epsilon$.

Similar to before, we have
\[
MC(\frac{c}{d}I_d, k) = \Pr[||\mu_1 - Y_1||^2 > \min_i ||\mu_i - Y_1||^2]
\]
where $Y_1 \sim N(\mu_1, I)$.

We can write
\[
Y_1 - \mu_1 = \begin{pmatrix}Z_1\\\vdots\\Z_d\end{pmatrix} = Z
\]
On a high-probability set, we have
\[
Y_1 - \mu_i = Z + ce_1 - ce_i + O(\epsilon)
\]

It follows that with probability greater than $1-\epsilon$, we have
\begin{align*}
MC(\frac{c}{d}I_d, k) &= \Pr[||\mu_1 - Y_1||^2 > \min_i ||\mu_i - Y_1||^2]
\\&= \Pr[2\sqrt{c}Z_1 > \min_i 2\sqrt{c}Z_i + 2c + O(\epsilon)]
\\&= \Pr[\min_i (Z_i + O(\epsilon)) < Z_1 - \sqrt{c}]
...
Hence in the limit,
\[
\lim_{d \to \infty} MC(\frac{c}{d}I_d, k) = 1 - \int_{\mathbb{R}} \Phi(\sqrt{c} - z)^{k-1} d\Phi(z)
\]
