...
\]
Let $A$ denote the event in which the above two sets of inequality hold.

In the event $A$, 
\[
\mu_i = \sqrt{c} e_i + O(\epsilon)
\]
given an appropriate orthonormal basis $e_1,\hdots, e_k$.  Here the $O(\epsilon)$
is understood to mean a quantity bounded in magnitude by a universal
constant (depending only on $c$) times $\epsilon$.
