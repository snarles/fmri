...
For $x > 0$ we have
\[
\Pr[||y+Z_i||^2_\Sigma < x] < K(\Sigma)x^{d/2}\phi_d(\overline{y}_x)
\]
where
\[
K(\Sigma) = \sqrt{\det(\Omega)} \frac{2\pi^{d/2}}{d\Gamma(d/2)}
\]
and $\overline{y}_x$ is defined as follows.

Consider the optimization problem
\[
\text{minimize} ||z-y||^2 \text{subject to}||z||_\Sigma^2 \leq x
\]
We assume $\epsilon$ sufficiently small such that the the solution
satisfies the hard constraint $||z||_\Sigma^2 = \epsilon$.  Take the
Lagrangian
\[
||z-y||^2 + \lambda z^T \Sigma z
\]
with $\lambda > 0$, and set the derivative equal to zero:
\[
...
\]
Let $\lambda^* > 0$ be the solution to
\[
x = y^T(I+\lambda\Sigma)^{-1}\Sigma(I+\lambda\Sigma)^{-1}y
\]
and define
\[
\overline{y}_x = y - (I+ \lambda^* \Sigma)^{-1}y.
\]

It follows that defining
\[
\underline{x}_p= x\text{ such that } K(\Sigma)\phi_d(\overline{y}_x) = 1-\sqrt[L]{1-p}\},
\]
we have
\[
\underline{x}_p < x_p.
...
\noindent\textbf{Lemma.}
\emph{For any $r \leq \sqrt{\gamma/x}$, where $\gamma$ is the largest eigenvalue of $\Sigma$, we have}
\[
\Pr[||y+Z_i||^2_\Sigma < x] < \text{Vol}(B_r) \phi_d(||y||+r)
\]
\emph{where $\phi_d(y) = (2\pi)^{-d/2}\exp(-y^2/2)$}.

The lemma follows from the fact that $z \in B_r$ implies
$||z||_\Sigma^2 < x$ and $\phi_d(y-z) \geq \phi_d(||y||+r)$.
Let $r^*$ be defined by
\[
r^* = \text{argmax}_r \text{Vol}(B_r) \phi_d(||y||+r)
\]
The lower bound then comes from choosing $r = \min(r^*, \gamma/x)$.

\[
\Pr[||y+Z_i||^2_\Sigma < x] > \text{Vol}(B_r) \phi_d(||y||+\min(r^*, \gamma/x))
\]
