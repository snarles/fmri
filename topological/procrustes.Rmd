...
p <- 5
q <- 2
SigmaX <- 3 * cov(randn(2*p, p)); SigmaY <- 3 * cov(randn(2 * p, p))
...
True bias and variance, skewness of test statistics.
...
Compare jackknife estimates to truth
...
Define studentized and bias-corrected statistics
...
## bias-corrected and studentized
bs_ <- function(theta) {
  newf <- function(res) {
    jj <- jackknife(res, theta)
    (theta(res) - jj$bias)/jj$sdv
  }
  newf
}
...
stat.T.s <- s_(stat.T)
stat.S.s <- s_(stat.S)
stat.T.bs <- bs_(stat.T)
stat.S.bs <- bs_(stat.S)
stat.T.ns <- s_(stat.T.nm)
stat.S.ns <- s_(stat.S.nm)
stat.T.nbs <- bs_(stat.T.nm)
stat.S.nbs <- bs_(stat.S.nm)
```

Figure 2: compare bias-corrected and studentized statistics to normal dist.
...
## non-corrected
mc.reps <- 1e2
thetas <- sampling_dist(sampler, theta, nX, nY, mc.reps, samples = TRUE)
hist(thetas[1, ], 
     breaks = seq(floor(min(thetas[1, ])), ceiling(max(thetas[1, ])), 0.5)
     )
xs <- seq(-7, 7, 0.1)
lines(xs, mc.reps/2 * dnorm(xs), col = "red")
## studentidized and bc
thetas <- sampling_dist(sampler, bs_(theta), nX, nY, mc.reps, samples = TRUE)
hist(thetas[1, ], 
     breaks = seq(floor(min(thetas[1, ])), ceiling(max(thetas[1, ])), 0.5)
     )
lines(xs, mc.reps/2 * dnorm(xs), col = "red")
names(res)
stat.T.bs(res)
mtext("Figure 2", outer = TRUE, cex = 1.5)
```

## Bootstrap distribution

In order to bootstrap the null, define

$$C = \frac{VU^T \hat{A} + \hat{B}}{2}$$

The null hypothesis is that

$$H_0: E[X] = UV^T C,\ E[Y] = C.$$

Therefore, compute residuals
...
The bootstrap resampler is implemented in `boot_sampler`.  The option `h0 = TRUE` resamples from the fitted null.

```{r}
boot_sampler <- function(res, h0 = FALSE) {
  p <- res$p; q <- res$q; dat <- res$dat; dat0 <- dat
  nX <- sum(dat[, 1] == 0); nY <- sum(dat[, 1] == 1);
  nX0 <- nX; nY0 <- nY
  rawX <- dat[dat[, 1]==0, -1]
  rawY <- dat[dat[, 1]==1, -1]
  residX <-t(t(rawX - colMeans(rawX)))
  residY <-t(t(rawY - colMeans(rawY)))  
  mus <- sample_moments(res)
  ress <- svd(mus$Ahat %*% t(mus$Bhat))
  R <- ress$v %*% t(ress$u)
  C <- (R %*% mus$Ahat + mus$Bhat)/2
  AC <- t(R) %*% C
  vecC <- as.numeric(C)
  vecAC <- as.numeric(AC)
  if (h0) {
    sampler <- function(nX = nX0, nY = nY0) {
      ...
      newX <- residX[sample(nX, nX, TRUE), , drop = FALSE]
      newY <- residY[sample(nY, nY, TRUE), , drop = FALSE]
      newX <- t(t(newX) + vecAC)
      newY <- t(t(newY) + vecC)
...
The sampling distribution of the bootstrap should resemble the original sampling distribution.

Figure 3: Example wth true null.
...
nX  <- 50; nY <- 50
mc.reps <- 100
samples_true <- sampling_dist(h0_small, theta, nX, nY, mc.reps, TRUE)
hist(samples_true)
res <- h0_small(nX, nY)
boot_s1 <- boot_sampler(res, h0 = FALSE)
samples_boot1 <- sampling_dist(boot_s1, theta, nX, nY, mc.reps, TRUE)
hist(samples_boot1)
boot_s0 <- boot_sampler(res, h0 = TRUE)
samples_boot0 <- sampling_dist(boot_s0, theta, nX, nY, mc.reps, TRUE)
hist(samples_boot0)
mtext("Figure 3", outer = TRUE, cex = 1.5)
```

Figure 4: Example wth true alernative.
...
nX  <- 50; nY <- 50
mc.reps <- 100
samples_true <- sampling_dist(h1_small, theta, nX, nY, mc.reps, TRUE)
hist(samples_true)
res <- h1_small(nX, nY)
boot_s1 <- boot_sampler(res, h0 = FALSE)
samples_boot1 <- sampling_dist(boot_s1, theta, nX, nY, mc.reps, TRUE)
...
For either the $T$ or $S$ statistic, we can test the global either by testing the norm, or doing multiple testing:

 * A single hypothesis test, $||T||_F^2 = 0$.
 * Multiple hypothesis tests, $T_{ij} = 0$.

To get p-values using bootstrap, one can either invert the confidence interval, or bootstrap the null.

```{r}

inverse_ci_test <- function(res, theta, mc.reps = 1000) {
  nX <- sum(res$dat[, 1] ==0); nY <- sum(res$dat[, 1]==1)
  boot_s1 <- boot_sampler(res, FALSE)
  thetas <- sampling_dist(boot_s1, theta, nX, nY, mc.reps, TRUE)
  pvs <- apply(thetas, 1, function(v) {
...
boot_null_test <- function(res, theta, mc.reps = 1000) {
  nX <- sum(res$dat[, 1] ==0); nY <- sum(res$dat[, 1]==1)
  boot_s0 <- boot_sampler(res, FALSE)
  theta0 <- theta(res)
  thetas <- sampling_dist(boot_s0, theta, nX, nY, mc.reps, TRUE)  
  pvs <- sapply(1:length(theta0), function(i) {
    v <- thetas[i, ]; v0 <- theta0[i]
    p1 <- (sum(v > v0) + 1)/(mc.reps + 1)
    p2 <- (sum(v < v0) + 1)/(mc.reps + 1)
    2 * pmin(p1, p2)
  })
  min(pvs) * length(pvs)
}
...
```{r, eval = FALSE}
nX <- 100; nY <- 100; mc.reps = 100
res0 <- h0_small(nX, nY)
res1 <- h1_small(nX, nY)

## stat T
c(inverse_ci_test(res0, stat.T, mc.reps), inverse_ci_test(res1, stat.T, mc.reps))
c(inverse_ci_test(res0, stat.T.s, mc.reps), inverse_ci_test(res1, stat.T.s, mc.reps))
c(inverse_ci_test(res0, stat.T.bs, mc.reps), inverse_ci_test(res1, stat.T.bs, mc.reps))
c(boot_null_test(res0, stat.T, mc.reps), boot_null_test(res1, stat.T, mc.reps))
c(boot_null_test(res0, stat.T.s, mc.reps), boot_null_test(res1, stat.T.s, mc.reps))
c(boot_null_test(res0, stat.T.bs, mc.reps), boot_null_test(res1, stat.T.bs, mc.reps))
## stat T norm
c(inverse_ci_test(res0, stat.T.nm, mc.reps), inverse_ci_test(res1, stat.T.nm, mc.reps))
c(inverse_ci_test(res0, stat.T.ns, mc.reps), inverse_ci_test(res1, stat.T.ns, mc.reps))
c(inverse_ci_test(res0, stat.T.nbs, mc.reps), inverse_ci_test(res1, stat.T.nbs, mc.reps))
c(boot_null_test(res0, stat.T.nm, mc.reps), boot_null_test(res1, stat.T.nm, mc.reps))
c(boot_null_test(res0, stat.T.ns, mc.reps), boot_null_test(res1, stat.T.ns, mc.reps))
c(boot_null_test(res0, stat.T.nbs, mc.reps), boot_null_test(res1, stat.T.nbs, mc.reps))
## stat S
c(inverse_ci_test(res0, stat.S, mc.reps), inverse_ci_test(res1, stat.S, mc.reps))
c(inverse_ci_test(res0, stat.S.s, mc.reps), inverse_ci_test(res1, stat.S.s, mc.reps))
c(inverse_ci_test(res0, stat.S.bs, mc.reps), inverse_ci_test(res1, stat.S.bs, mc.reps))
c(boot_null_test(res0, stat.S, mc.reps), boot_null_test(res1, stat.S, mc.reps))
c(boot_null_test(res0, stat.S.s, mc.reps), boot_null_test(res1, stat.S.s, mc.reps))
c(boot_null_test(res0, stat.S.bs, mc.reps), boot_null_test(res1, stat.S.bs, mc.reps))
## stat S norm
c(inverse_ci_test(res0, stat.S.nm, mc.reps), inverse_ci_test(res1, stat.S.nm, mc.reps))
c(inverse_ci_test(res0, stat.S.ns, mc.reps), inverse_ci_test(res1, stat.S.ns, mc.reps))
c(inverse_ci_test(res0, stat.S.nbs, mc.reps), inverse_ci_test(res1, stat.S.nbs, mc.reps))
c(boot_null_test(res0, stat.S.nm, mc.reps), boot_null_test(res1, stat.S.nm, mc.reps))
c(boot_null_test(res0, stat.S.ns, mc.reps), boot_null_test(res1, stat.S.ns, mc.reps))
c(boot_null_test(res0, stat.S.nbs, mc.reps), boot_null_test(res1, stat.S.nbs, mc.reps))
```

# Better bootstrap intervals

Using BCA and ABC method for inverse CI tests.

The BCA alpha is given by

$$
\alpha_{BCA} = \Phi\left(z_0 + \frac{z_0 + z^{(\alpha)}}{1 - a (z_0 + z^{(\alpha)})}\right)
$$

Hence, to invert the BCA interval, use

$$
\alpha = \Phi\left(
\frac{(1-az_0)(z^{BCA} - z_0) - z_0}{1 + a (z^{BCA} - z_0)}
\right)
$$

```{r}
alpha_bca <- function(alpha, z0, acc) {
  za <- qnorm(alpha)
  pnorm(
    z0 + (z0 + za)/(1 - acc * (z0 + za))
    )
}
...
alpha_bca(0.1, 0.2, 0.01)
inv_alpha_bca(0.19251559, 0.2, 0.01)
```

Code for BCA inverted p-values.
