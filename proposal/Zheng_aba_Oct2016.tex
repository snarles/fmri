...
\begin{frame}
\frametitle{Motivation 1: Generalizing to identical replicate}
\emph{ What can be inferred from the classification accuracy?}
\begin{itemize}
...
Can we predict $\text{ABA}_{100}$ given data from 30 classes?  See Z., Achanta and Benjamnini (2016).
\begin{figure}
\centering
...
\begin{frame}
\frametitle{An identity} Let's go over the result more slowly.
Suppose $k = 3$.  For the following one-dimensional example, we have
plotted $p(y|x_i)$ for $i = \{1,2,3\}$.

\begin{align*}
\text{BA}(x_1,x_2,x_3) &= \sum_i \Pr[x_i]\Pr[\text{zone} i| Y \sim p(y|x_i)]
...
$\frac{1}{2\sqrt{k}}$ & 0.25 & 0.194 & 0.167 & 0.150 & 0.136 & 0.126 & 0.118\\\hline
Worst-case sd & 0.353 & 0.289 & 0.250 & 0.223 & 0.204 & 0.189 & 0.177
\end{tabular}
\end{frame}
...
\item Step 2: Use $\text{TA}_k$ to infer the generalization accuracy $\text{GA}_k$
%\[
%\text{GA}_k \geq \text{TA}_k - z_\alpha \sqrt{\frac{\text{TA}_k(1-\text{TA}_k)}{n_{test}}} \text{ with probability } \geq 1-\alpha
%\]
\item Step 3: The generalization accuracy is a lower bound on the Bayes accuracy,
\[
\text{GA}_k \leq \text{BA}_k
\]
\item Step 4: Use $\text{BA}_k$ to infer the average Bayes accuracy $\text{ABA}_k$
%\[
%\text{ABA}_k \geq \text{BA}_k - \frac{1}{2\sqrt{\alpha k}}  \text{ with probability } \geq 1-\alpha
%\]
\item Step 5: Obtain a lower bound on $I(X; Y)$ from $\text{ABA}_k$!
\end{itemize}
We already know how to do steps 1-4; now we discuss step 5.
\end{frame}
