...
Suppose we have data from \emph{randomized classification}
\begin{enumerate}
\item Can we infer $k$-class average Bayes error?
\item Can we infer mutual information?
\end{enumerate}
\begin{center}
\includegraphics[scale = 0.3]{searchlight_windowsize.png}
\end{center}
\end{frame}
...
\section{Inference of average Bayes accuracy}
...
Converting a lower confidence bound (LCB) for $\text{BA}_k$ to an LCB on $\text{ABA}_k$ boils down to the following problem:
\vspace{0.2in}
...
\section{Inferring mutual information}
