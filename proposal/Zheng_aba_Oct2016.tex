...
and
\[
\text{ABA}_k[p(x, y)] = \frac{1}{k}\int \left[\int \max_{i=1}^k p(x_i|y) dx_1 \cdots dx_k\right] dy.
\]
...
\[
\text{ABA}_k[p(x, y)] = \frac{1}{k}\int \underbrace{\left[\int \max_{i=1}^k p(x_i|y) dx_1 \cdots dx_k\right]}_{\text{CA}_k[p(x|y)]} dy.
\]
\end{frame}
