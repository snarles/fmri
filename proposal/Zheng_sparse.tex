...
Identification risk $\text{IdRisk}_k[p(x, y)]$ and mutual information $\text{I}[p(x, y)]$ are both \emph{functionals} of $p(x, y)$.
\[
\text{IdAcc}_k[p(x, y)] = 1 - \frac{1}{k} \int p_X(x_1)\hdots p_X(x_k) \max_{i=1}^k p(y|x_i)  dx_1\hdots dx_k dy.
\]
\[
...
\]
\[
\text{IdAcc}_k[q(x)] \stackrel{def}{=} \frac{1}{k}\int_{[0, 1]^k} \max_{i=1}^k q(x_i) dx_1 \cdots dx_k
\]
...
We now pose the question: how do we find $q(x)$ which maximizes $\text{IdAcc}_k[q(x)]$ subject to $\text{I}[q(x)] \leq \iota$?
...
\[
\text{maximize}_{q(x) \in \mathcal{Q}}\text{ IdAcc}_k[q(x)]\text{ subject to }\int_0^1 q(x) dx = 1\text{ and }\text{I}[q(x)] \leq \iota.
\]
\end{frame}
...
\begin{align*}
\mathcal{L}[q(x)] &= -\text{IdAcc}_k[q(x)] + \lambda \int_0^1 q(x) dx + \nu \text{I}[q(x)]
