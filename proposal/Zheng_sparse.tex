...
\item Goal: estimate the informational correlation
\[
\text{Cor}_{Info}(\vec{X}^{(i)}, \vec{X}^{(j)})
\]
between each pair, $i \neq j$.\pause
\item Conclusion: find groups which are high predictive of each other--this may have biological significance.\pause 
\item Also: we will show that our method is \emph{robust} to rotations.
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Correlations between time series}
\begin{columns}
\begin{column}{0.5\textwidth}
$\text{Cor}_{Info}$ (using OLS)\\
...
{\tiny *Witten and Tibshirani 2009, {\tt PMA::CCApermute}}
\end{frame}
...
\begin{center}
\includegraphics[scale = 0.4]{../idloss/info_cor_robustness.pdf}
\end{center}
\end{column}
...
Using sparse CCA* 
\begin{center}
\includegraphics[scale = 0.4]{../idloss/cca_p_robustness.pdf}
\end{center}
\end{column}
