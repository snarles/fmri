...
\]
\item Simply using plugging in kernel density estimate $\hat{p}(x, y)$ leads to bias (Beirlant et al. 2001)
\item Jackknifed estimate reduces bias (Ivanov and Rozhkova 1981)
%\[
%\hat{I}(X; Y) = \frac{1}{n} \sum_{i=1}^n \log \left(\frac{\hat{p}_{-i}(x_i, y_i)}{\hat{p}_{-i}(x_i)\hat{p}_{-i}(y_i)}\right)
%\]
\item Alternative to KDE: nearest neighbors (Mnatsakanov et al. 2008, Goria et al. 2005, Singh et. al. 2003)
\end{itemize}
\end{frame}
...
I(X; Y) \geq g(p)
\]
where $g$ is a function that we derive theoretically. 
\end{enumerate}
\end{frame}
