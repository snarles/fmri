...
\end{center}
\begin{itemize}
\item Introduced in Shannon's 1948 paper, ``A mathematical theory of communication''
\item Mutual information measures nonlinear dependence
\end{itemize}
\end{frame}
...
Pearson correlation
\begin{itemize}
\item[+] Easier to interpret \\(0 = independence,\\ 1 = perfect correlation)
\item[-] Only applies to univariate $X$, $Y$
\item[-] Only captures linear associations
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
Mutual information
\begin{itemize}
\item[+] Captures nonlinear associations
\item[+] Extends to arbitrarily many dimensions
\item[+] Invariant to nonlinear scaling
\item[-] Harder to estimate in data
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\] \pause
\item Define the ``Shannon correlation''
\[
\text{Cor}_{Shannon}(X, Y) = \sqrt{1-e^{2I(X; Y)}}
\]\pause
\item Then $\text{Cor}_{Shannon}(X, Y) \in [0,1]$.
...
Example with $\text{Cor}_{Pearson}(X, Y) = \text{Cor}_{Shannon}(X, Y) = 0.44$.
\begin{center}
...
\begin{frame}
\frametitle{Difficulty of estimating $I(X; Y)$}
Example with $\text{Cor}_{Pearson}(X, Y) = \text{Cor}_{Shannon}(X, Y) = 0.2$.
\begin{center}
