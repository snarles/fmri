...
li <- floor(0.1 * data.reps); ui <- floor(0.9 * data.reps)
m_i <- floor(0.5 * data.reps)
lowers <- t(apply(ress, c(2, 3), function(v) sort(v)[li]))
uppers <- t(apply(ress, c(2, 3), function(v) sort(v)[ui]))
...
save(allresults, file = 'info_theory_sims/fig4.Rdata')