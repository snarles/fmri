...
run_simulation <- function(Bmat, m.folds, k.each, r.each, r.train, alpha = 0.05) {
  ...
  for (j in 1:m.folds) {
    ...
    mcs[j] <- mc
    mi_cms[j] <- mi_cm
    Xall <- rbind(Xall, Xmat)
    Yall <- rbind(Yall, Y)
  }
  Bhat <- sapply(1:ncol(Xall), function(i) {
    res <- glm(Yall[, i] ~ Xall + 0, family = "binomial")
    coefficients(res)
  })
  ...
  ## lower confidence bound for BA
  (ba_l <- 1 - abe0 - sqrt(-log(alpha/4)/(2 * m.folds * k.each * r.test)))  
  (aba_l <- ba_l - sqrt(1/(2 * alpha * k.each * m.folds)))
  (i_l <- aba_to_mi_lower(k.each, aba_l))
  (i_l2 <- aba_to_mi_lower(k.each, 1-abe))
  ## answer
