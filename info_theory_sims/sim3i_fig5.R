...
## data params
N <- 80000
ks <- c(5, 10, 15, 20, 30, 40, 50, 60, 70, 80)
...
tab2 <- data.frame(I = res2[, 4], K = K)

res3 <- do.call(cbind, ress)
mses <- colSums((res3 - mi_true)^2)/data.reps
mses_ls <- mses[(1:length(ks)) * 9 - 6]
mses_0 <- mses[(1:length(ks)) * 9 - 5]


boxplot(I ~ K, data = tab1, ylim = c(0.5, 2.8), xlab = "K")
abline(mi_true, 0, lty = 2, col = "red", lwd = 3)
text(1.5, 2.8, "RMSE x 100 = ", col = "red", cex = 0.8)
for (i in 1:length(ks)) text(i, 2.5, format(round(mses_ls[i] * 1e2, 1)), col = "red")
#abline(est_ls[2], 0)

boxplot(I ~ K, data = tab2, ylim = c(0.5, 2.8), xlab = "K")
abline(mi_true, 0, lty = 2, col = "red", lwd = 3)
text(1.5, 2.8, "RMSE x 100 = ", col = "red", cex = 0.8)
for (i in 1:length(ks)) text(i, 2.5, format(round(mses_0[i] * 1e2, 1)), col = "red")
