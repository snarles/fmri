...
sigma2s <- seq(0.05, 1.1, 0.05)^2
ac_by_sig2 <- unlist(mclapply(sigma2s, function(s) {
...
t1 <- proc.time()
res <- mclapply(1:480, subfun, mc.cores = 6)
proc.time() - t1
...
matplot(sqrt(sigma2s), sqrt(final_pred_err_mat), type = "l", lwd = 3)
legend(10, 0, legend = names(basis_vecs), col = 1:6, lty = 1:6)
plot(sqrt(sigma2s), true_accs[, K])

save(true_accs, all_final_preds, final_pred_err_mat, file = "approximation/mcgs2fovs_02.rda")
