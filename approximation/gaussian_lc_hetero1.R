...
c1s <- unlist(mclapply(Sigma1s, function(s)
  build_mc2c_table(s, goal=1/4, func = mc2c_2, mc.reps = 1e6, tol=1e-4), mc.cores = mcc))
...
maxK <- 10
res <- mclapply0(Sigma1s, function(s) mcK(s, maxK, N = 200, mc.reps=1e4), mc.cores = mcc)
mat <- do.call(cbind, res)
matplot(mat, type = "l", col = rainbow(length(Sigma1s)), lty = 1, lwd = 1)
res0 <-mcK(Sigma0, maxK, N = 200, mc.reps = 1e4)
