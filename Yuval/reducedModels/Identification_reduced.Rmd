...
source('identification_functions.R')
...
feat_mean_vec = attr(c_trainF_full,'scaled:center')
...
save(file = sprintf('trainDataReduced%d.rda', max_pyr_level), trainY , sel_trainF)
...
if(formNewRules){
  ...
  load(sprintf('sel_%d_pred_rules.RData',max_pyr_level))
} 
```
...
newCondVar =genCondVar(1:1302,usevox,ind_struct = vox_prediction_rules_nor)
